# Domain-Driven Design Cards

Download as zip-file, fork or clone the repository.

Lade als Zip-Datei herunter, forke oder klone das Repository.

You will find a detailed description in both **_ENGLISH_** and **_GERMAN_** below.

Eine detaillierte Beschreibung findest du sowohl in **_ENGLISCH_** als auch in **_DEUTSCH_** unten.

---

## English Version

### Release Notes

* `UPDATE` cards of tactical architecture game
* `NEW` version 1 of descriptions for all DDD cards games in german

### Enable Domain-Driven Design with Gamification

Domain-Driven Design Cards offer valuable methodological support for a range of strategic and 
tactical Domain-Driven Design workshops, and they're available for free. This initiative is a collaborative 
project with [nilsbert](https://github.com/nilsbert). For more information about us, you can explore the [website](http://www.architecture-enablers.de).

With Domain-Driven Design Cards, you can engage in activities such as the Context Mapping Game, 
the Strategic Classification Game, the Bounded Context Game, and the Architecture Game. If you're looking for a comprehensive guide 
on how to use them, please consult the following sections.

### Bounded Context Game

Identify domain-related boundaries (subdomains, bounded contexts) within your business domain.

#### Overview Card Deck

![Overview Card Deck Bounded Context Game](./cards/Bounded_Context_Game_Cards_Overview.png)

### Context Mapping Game

Define dependencies between bounded contexts.

#### Overview Card Deck

![Overview Card Deck Context Mapping Game](./cards/Context_Mapping_Game_Cards_Overview.png)

### Strategic Classification Game

Determine the strategic importance of your subdomains and bounded contexts.

#### Overview Card Deck

![Overview Card Deck Strategic Classification Game](./cards/Strategic_Classification_Game_Cards_Overview.png)

### Tactical Architecture Game

Define the tactical architectural construction plan collaboratively within the development team.

#### Overview Card Deck

![Overview Card Deck Tactical Architecture Game](./cards/Tactical_Architecture_Game_Cards_Overview.png)

### The Domain-Driven Design Cards Game Collection in English

Coming soon ...

[//]: # (1. [Enabling Domain-Driven Design with Gamification]&#40;./mkdocs-en.md&#41;)

[//]: # (2. [The Bounded Context Game]&#40;./docs/docs/bounded-context-game-en.md&#41;)

[//]: # (3. [The Strategic Classification Game]&#40;./docs/docs/scg/strategic-classification-game-en.md&#41;)

[//]: # (4. [The Context Mapping Game]&#40;./docs/docs/cmg/context-mapping-game-en.md&#41;)

[//]: # (5. [The Tactical Architecture Game]&#40;./docs/docs/docs/tag/tactical-architecture-game-en.md&#41;)

[//]: # (6. [Architecture Enabling Based On Collaborative Design Decisions]&#40;./docs/docs/tag/tag-overview.md&#41;)

---

## Deutsche Version

### Release Notes

* `UPDATE` Karten des Tactical Architecture Game
* `NEW` Version 1 der Beschreibungen für alle DDD-Kartenspiele auf Deutsch

### Enable Domain-Driven Design mit Gamification

Domain-Driven Design Cards bieten wertvolle methodische Unterstützung für eine Reihe von strategischen und 
taktischen Domain-Driven Design Workshops und sind kostenlos verfügbar. Diese Initiative ist ein gemeinschaftliches 
Projekt mit [nilsbert](https://github.com/nilsbert). Weitere Informationen über uns findest du auf der [Website](www.architecture-enablers.de).

Mit Domain-Driven Design Cards kannst du Aktivitäten wie das Context Mapping Game, 
das Strategic Classification Game, das Bounded Context Game und das Architecture Game durchführen. Wenn du einen umfassenden Leitfaden 
zur Verwendung suchst, konsultiere bitte die folgenden Abschnitte.

### Bounded Context Game

Identifiziere domänenbezogene Grenzen (Subdomänen, Bounded Contexts) innerhalb deiner Business-Domäne.

#### Übersicht Kartendeck

![Übersicht Kartendeck Bounded Context Game](./cards/Bounded_Context_Game_Cards_Overview.png)

### Context Mapping Game

Definiere Abhängigkeiten zwischen Bounded Contexts.

#### Übersicht Kartendeck

![Übersicht Kartendeck Context Mapping Game](./cards/Context_Mapping_Game_Cards_Overview.png)

### Strategic Classification Game

Bestimme die strategische Bedeutung deiner Subdomänen und Bounded Contexts.

#### Übersicht Kartendeck

![Übersicht Kartendeck Strategic Classification Game](./cards/Strategic_Classification_Game_Cards_Overview.png)

### Tactical Architecture Game

Definiere den taktischen Architektur-Konstruktionsplan kollaborativ innerhalb des Entwicklungsteams.

#### Übersicht Kartendeck

![Übersicht Kartendeck Tactical Architecture Game](./cards/Tactical_Architecture_Game_Cards_Overview.png)

### Die Domain-Driven Design Cards Game Kollektion auf Deutsch

[Enabling Domain-Driven Design und Softwarearchitektur mit Gamification](https://matthiaseschhold.github.io/domain-driven-design-cards/introduction/introduction-de.html)
